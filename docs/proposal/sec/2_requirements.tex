...
\item Voxel Carving: We will implement a classical space carving pipeline in C++. The algorithm will initialize a dense voxel grid around the object and iteratively remove voxels whose projections are inconsistent with the object silhouettes in the calibrated camera views. We will use the resulting voxel model as a classical reconstruction baseline and as a possible initialization source for the Gaussian Splatting pipeline.

\item 3D Gaussian Splatting: The main goal of the project is to reimplement the core components of 3D Gaussian Splatting~\cite{Kerbl2023GaussianSplatting} in C++. We will represent the scene as a set of anisotropic 3D Gaussians with optimizable position, scale, rotation, opacity, and color parameters. In contrast to the original paper, we will obtain camera intrinsics and extrinsics from our ArUco/OpenCV calibration pipeline rather than from a structure-from-motion reconstruction. We will implement a differentiable splatting renderer that projects the Gaussians into the calibrated camera views and optimizes their parameters by minimizing the difference between rendered and reference images.
...
\item Evaluation: We will compare Voxel Carving and 3D Gaussian Splatting quantitatively and qualitatively on the same custom datasets. We will evaluate reconstruction quality, runtime, memory consumption, and robustness across different object types and material properties.
...
\item nlohmann/json or yaml-cpp: We will use a structured configuration format to store dataset paths, camera parameters, calibration results, and experiment settings.
