...
\item \textbf{Week 1 (9.6--15.6):} We will capture initial multi-view datasets, estimate camera intrinsics and extrinsics using ArUco markers and OpenCV, and prepare a basic dataset structure containing images, camera parameters, and foreground masks. We will also begin implementing a simple reference voxel-carving prototype based on binary silhouettes.

\item \textbf{Week 2 (16.6--22.6):} We will refine the reference voxel carving pipeline and test it on the captured datasets. This includes improving silhouette handling, selecting a suitable voxel grid resolution, exporting the reconstruction for visualization, and comparing the result against Open3D on the same inputs. In parallel, we will start setting up the basic data structures required for the Gaussian Splatting implementation.
...
\item \textbf{Week 6 (14.7--21.7):} We will run the final experiments and compare the reference voxel carving baseline, Open3D, our Gaussian Splatting implementation, and gsplat. The evaluation will consider visual quality, reconstruction behavior, runtime, memory usage, and robustness for different object types. Finally, we will summarize the results, discuss limitations, and prepare the final report and presentation.
