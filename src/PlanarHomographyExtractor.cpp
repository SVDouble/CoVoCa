...
PlanarHomographyExtractor::PlanarHomographyExtractor(
  ...
  for (size_t i = 0; i < config.referenceIds.size(); ++i) {
    ...
    if (i < config.referenceCorners.size() &&
      ...
      std::cerr << "[Planar] WARNING: Marker " << id
                << " has invalid corners.\n";
    }
  }
...
cv::Mat PlanarHomographyExtractor::extract(const cv::Mat &inputImage) {
  ...
  cv::aruco::ArucoDetector(m_dictionary)
      .detectMarkers(inputImage, corners, ids);
  ...
  cv::warpPerspective(inputImage, warpedColor, H,
                      m_config.referenceImage.size(), cv::INTER_LINEAR);
  ...
  cv::Mat maskOriginal;
  cv::Mat H_inv = H.inv();
  cv::warpPerspective(maskWarped, maskOriginal, H_inv, inputImage.size(),
                      cv::INTER_NEAREST);
...
cv::Mat PlanarHomographyExtractor::createBoardMask(const cv::Size &size) {
  ...
  if (allCorners.empty()) {
    std::cerr << "[Planar] WARNING: No marker corners for board mask. Using "
                 "full image.\n";
  ...
  cv::fillPoly(mask, std::vector<std::vector<cv::Point>>{expandedHull},
               cv::Scalar(255));
  return mask;
}
...
cv::Mat PlanarHomographyExtractor::createMarkerMask(const cv::Size &size) {
  ...
  for (const auto &pair : m_refMarkerCorners) {
    ...
    cv::fillPoly(markerMask, std::vector<std::vector<cv::Point>>{pts},
                 cv::Scalar(0));
  }
  return markerMask;
}
