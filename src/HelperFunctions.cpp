...
int calculateFlattenedIndex(const Eigen::Vector3i &size, int x, int y, int z) {
...
void visualizeSilhouette(const cv::Mat &original, const cv::Mat &silhouette,
  ...
  // Convert silhouette from grayscale to color so that both have matching
  // number of channels for concatenation
...
std::vector<cv::Mat> loadImages(std::string _folder) {
  ...
  // important to load in same order as cameras since index in vector detemines
  // which camera is associated
  ...
  for (const auto &path : files) {
    ...
    std::cout << "Loaded image no. " << i << " from " << path.filename()
...
std::vector<Camera> loadCameras(const std::string &filename) {
  ...
  for (int i = 0; i < num_cameras; ++i) {
    ...
    std::cout << "Loaded camera no. " << i << " associated with image "
              << image_name << std::endl;
  }
