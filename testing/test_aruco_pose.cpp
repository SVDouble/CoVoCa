...
int main(int argc, char** argv) {
    // 1. Set up the Aruco board parameters based on the cat dataset calibration_result.yaml
    int markers_x = 4;
    int markers_y = 5;
    ...
    cv::Mat camera_matrix = (cv::Mat_<double>(3, 3) << 
        870.195780, 0.000000, 634.411075,
        0.000000, 862.140009, 499.328103,
        0.000000, 0.000000, 1.000000);
        
    cv::Mat dist_coeffs = (cv::Mat_<double>(5, 1) << 
        0.170929, -0.949321, -0.020620, 0.000191, 1.597958);
    ...
    if (success) {
        ...
        if (!marker_ids.empty()) {
            ...
            for (size_t i = 0; i < rvecs.size(); ++i) {
                ...
                cv::drawFrameAxes(result_image, camera_matrix, dist_coeffs, rvecs[i], tvecs[i], marker_length * 0.8f);
            }
        }
        
        // Draw coordinate axes for the WHOLE board (Global World Coordinate System for Voxel Carving)
        // This is the most important one! It shows the origin (0,0,0) of your 3D reconstruction.
        cv::drawFrameAxes(result_image, camera_matrix, dist_coeffs, rvec, tvec, 0.12f);
